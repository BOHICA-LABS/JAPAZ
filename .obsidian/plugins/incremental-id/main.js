...
var IncrementalIdPluginSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  addAddIdDefinitionSection() {
    const isIdDefinitionValid = (def) => {
      ...
      if (def.prefix.length < 2 || !/^[A-Z]*$/.test(def.prefix) || this.config.configuration.idDefinitions.find(({ prefix }) => prefix === def.prefix)) {
...
var IncrementalIdConfiguration = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
